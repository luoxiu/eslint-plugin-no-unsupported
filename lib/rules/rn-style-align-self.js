    ...
    return {
      Property(node) {
        if (node.key.name === 'alignSelf' || node.key.value === 'alignSelf') {
          ...
          const regexp = /^(auto|normal|stretch|(((first|last) )?baseline)|(((unsafe|safe) )?(center|start|end|self-start|self-end|flex-start|flex-end)))$/;
          ...
          if (regexp.test(value) && !supportedValues.includes(value)) {
            context.report({
              node,
